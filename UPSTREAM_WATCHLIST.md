...
| Inline view-file button and previous/next file-history navigation | Missing upstream. Upstream has file history views (PRs 52634 and 56500 merged), a `file_filter` on `CommitView`, and a Git Graph view (`crates/git_ui/src/git_graph.rs`) | Retire `file-history-navigation` when both land; narrow it as pieces land |
