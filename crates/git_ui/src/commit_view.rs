...
fn navigate_file_history(
    ...
    let receiver = repository.update(cx, |repo, _| {
        repo.file_history_shas(file_path.clone(), None)
    });
    let repo_weak = repository.downgrade();
    window
        .spawn(cx, async move |cx| {
            let history_shas = match receiver.await {
                Ok(Ok(history_shas)) => history_shas,
                _ => return,
            };
            let Some(current_ix) = history_shas
                .iter()
            ...
            let Some(target_sha) = history_shas.get(target_ix) else {
                return;
            };
