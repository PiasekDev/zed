...
impl GitFileDiffView {
    ...
    pub fn view_last_commit(&mut self, window: &mut Window, cx: &mut gpui::Context<Self>) {
        ...
        let Some(repo) = repository.upgrade() else {
            return;
        };
        let load_task = repo.update(cx, |repo, _| {
            repo.file_history_shas(repo_path.clone(), Some(1))
        });

        cx.spawn_in(window, async move |_, cx| {
            let history_shas = load_task.await??;
            let Some(first_entry_sha) = history_shas.first() else {
                return Ok(());
            };
            let commit_sha = first_entry_sha.to_string();
