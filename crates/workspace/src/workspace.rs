...
impl Workspace {
    ...
    fn apply_window_title(&mut self, window: &mut Window, cx: &mut App) {
        ...
        // tell them apart in the dock; items without a path (terminals, graph
        // tabs) get nothing from the title template otherwise.
        if self.detached_from.is_some()
            && let Some(tab_text) = active_item
                .as_ref()
                .map(|item| item.tab_content_text(0, cx))
                .filter(|text| !text.is_empty())
            && !title.contains(tab_text.as_ref())
        {
            title.push_str(" — ");
